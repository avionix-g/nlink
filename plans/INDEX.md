...
| [154](154-0.16-nlink-macros-plan.md) | New `nlink-macros` proc-macro crate: `#[derive(GenlMessage/GenlCommand/GenlAttribute/GenlEnum/NetlinkAttrs)]` + `#[genl_family]` | ~7.5 days | 9 | 🟡 | – | Phase 1 done: crate scaffold + `#[derive(GenlCommand)]` + 7 runtime tests + 5 trybuild compile-fail cases. Remaining: GenlAttribute (Phase 2) + GenlEnum (Phase 2) + GenlMessage + NetlinkAttrs (Phase 3) + #[genl_family] attr macro (Phase 4) + Connection::send_typed (Phase 5) + example + re-export. **Publish before nlink** when the cycle cuts |
