...
| [154](154-0.16-nlink-macros-plan.md) | New `nlink-macros` proc-macro crate: `#[derive(GenlMessage/GenlCommand/GenlAttribute/GenlEnum/NetlinkAttrs)]` + `#[genl_family]` | ~7.5 days | 9 | 🟡 | – | Phases 1+2+3a+3b+4+5+6 done: 5 derive/attr macros + `GenlFamily` send-time trait + `Connection::<F: GenlFamily>::send_typed<M, R>` / `dump_typed_stream<M, R>` generic dispatch + worked example (`examples/macros/define_taskstats.rs`) + recipe (`docs/recipes/define-your-own-genl-family.md`). As a Phase 6 side benefit: the six in-tree GENL families' hand-rolled per-family `new_async()` inherent methods collapsed into a single generic `Connection::<P: AsyncConstructible>::new_async()`, which is also what plugs macro-defined families into the canonical constructor for free. 23 runtime tests in nlink::macros + 14 in nlink-macros. Remaining: `#[derive(NetlinkAttrs)]` (nested-group support — pure follow-up) + final re-export polish + CHANGELOG framing (Phase 7). **Publish before nlink** when the cycle cuts |
