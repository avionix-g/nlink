...
|------|-------|--------|-------|--------|-------|-------|
| [170](170-nft-send-batch-hang-investigation-plan.md) | `Connection::<Nftables>::send_batch` seq filter + end-seq termination — fixes the 0.16 cut's CI hang; un-ignores 7 `nftables_reconcile::*` tests | ~2.5 h | 1 | 🟢 | #4 | Shipped `dc4c103` + un-ignore `b9c8eb6`. 4 of 7 reconcile tests now pass; 3 re-ignored under [Plan 178](178-nftables-diff-body-bytes-false-positive-plan.md). |
| [171](171-default-connection-timeout-plan.md) | Default 30s operation timeout on `Connection<P>` — opt-out via `.with_no_timeout()`; closes the "hidden hang" class that masked Plan 170 | ~3 h | 2 | 🟢 | #4 | Shipped `2a1251f`; integration timeout tests adjusted `66210c0`. |
...
| [174](174-ci-observability-plan.md) | `tracing-subscriber` in integration test harness + `nf_flow_table` modprobe + ignored-tests catalog | ~1.5 h | 5 | 🟡 | #4 | All three phases landed locally; awaiting CI confirmation. |
...
| [178](178-nftables-diff-body-bytes-false-positive-plan.md) | `NftablesDiff` body-bytes comparison flags unchanged rules as `to_replace`; un-ignores the 3 reconcile tests blocked by it | ~2-3 h | 8 | ⚪ | – | Surfaced after Plan 170's send_batch fix exposed real diff behavior on idempotent reapply. |
