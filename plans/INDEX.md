...
| [154](154-0.16-nlink-macros-plan.md) | New `nlink-macros` proc-macro crate: `#[derive(GenlMessage/GenlCommand/GenlAttribute/GenlEnum/NetlinkAttrs)]` + `#[genl_family]` | ~7.5 days | 9 | 🟡 | – | Phases 1+2+3a+3b+4 done: 5 derive/attr macros covering everything a downstream author needs to declare a complete GENL family in ~20 lines (codec enums + message body + family marker with all 4 trait impls + family_id resolver). 17 runtime tests in nlink::macros + 14 in nlink-macros. nlink → nlink-macros wired. Remaining: `#[derive(NetlinkAttrs)]` (nested-group support — pure follow-up) + `Connection::send_typed` generic dispatch (Phase 5) + example + recipe (Phase 6). **Publish before nlink** when the cycle cuts |
