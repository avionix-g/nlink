...
| [185](185-nftables-subscribe-with-resync-plan.md) | `Connection<Nftables>::{into_events_with_resync, subscribe_all_with_resync}(factory)` — kube-rs-shaped watcher with built-in ENOBUFS recovery; both owned + borrowed forms | ~5 h | 6 | ⚪ | **Plan expanded 2026-05-30** — maintainer authorized breaking changes; lifetime-generic refactor of `events_with_resync` + `NftablesEvent::{NewSet,DelSet}` parsing now bundled. Both owned (`'static`, spawn-able, primary) and borrowed (`'_`, convenience) forms ship together. |
