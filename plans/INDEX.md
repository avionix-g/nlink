...
| [149](149-0.16-streaming-dump-api-plan.md) | `dump_stream<T>` generic + `stream_routes/links/neighbors/fdb` + TC streams + typed config streams (XFRM SA/SP, conntrack) | ~3 days | 4 | 🟡 | – | Foundation `dump_stream<T>` + four rtnetlink wrappers (links/routes/neighbors/addresses) landed. TC wrappers (`stream_qdiscs`/`stream_classes`/`stream_filters`) landed as a follow-up. Typed-config streams (XFRM SA/SP, conntrack, nft rules) still deferred — same mechanical pattern, will be added when those subsystems get their dump-heavy use case |
| [153](153-0.16-kernel-feature-bundle-plan.md) | XFRM IPsec offload (`XFRMA_OFFLOAD_DEV`) + Devlink rate + port-function-state + TX H/W shaping (`net_shaper` GENL) | ~5 days | 5 | 🟡 | – | §4.1 XFRM offload + §4.2 Devlink rate / port-function-state landed. **§4.3 net_shaper GENL family fully unblocked** after Plan 154 Phase 8 closeout (all field shapes — scalar enums, repeated enums, bitflags, signed ints, nested groups — now expressible via the macros). Ready to start |
...
| [156](156-0.16-dpll-genl-family-plan.md) | DPLL netlink family (kernel 6.7+) — first in-tree user of `nlink-macros`. ~130 lines of macro-derived declaration for the full family | ~2 days | 10 | 🟡 | – | **Phases 1-4 + most of 6 done.** Family marker + 11 typed enums (12 commands + 14 device attrs + 31 pin attrs + 8 value GenlEnums + DpllFeatureState 0-based + DpllPinCapabilities bitflags) + device-side messages + pin-side messages with 2 nested attribute groups + Connection methods (get/dump/set for devices and pins) + runnable example + recipe. 21 unit tests in `genl::dpll::*`. **Validated against a live kernel** — family resolution works, dump path produces clean EPERM on the unprivileged side (kernel ACL). Phase 5 (multicast monitor + `DpllEvent` stream) deferred — needs new GENL group-ID-resolution infrastructure |
...
| [160](160-example-registry-audit.md) | Audit: every example .rs must be registered in `Cargo.toml`; CI gate to enforce + per-file resolution catalog for 9 orphans | ~0.5 day (script + catalog); per-orphan triage separate | 14 | 🟡 | – | Triggered by a code-review pass during the 0.16 cycle. Script `scripts/audit-example-registration.sh` ships in tree (dormant — not yet wired into CI because 9 known orphans would immediately break it). Catalog + per-file category (Rename / Phantom / Obsolete) in the plan file. Convention added to CLAUDE.md. **Remaining**: maintainer triages the 9 orphans (one or more follow-up commits), then the workflow stanza wires the script as a CI gate |
