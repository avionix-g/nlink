...
| [157](157-0.16-nftables-declarative-config-plan.md) | Declarative `NftablesConfig` — `diff + apply + reconcile` mirroring `NetworkConfig`, atomic-via-`Transaction` commit, canonicalized rule equivalence | ~3 days | 12 | 🟡 | – | Core types + diff landed. **Atomicity landed**: Transaction extended with `del_chain` / `del_rule` / `add_flowtable` / `del_flowtable`; `NftablesDiff::apply` now bundles every diff op into a single `NFNL_MSG_BATCH_BEGIN ... END` round-trip (kernel commits or rolls back the whole batch — no half-applied state observable). One narrow gap: tables with `NFT_TABLE_F_DORMANT`/`_OWNER`/`_PERSIST` still use the imperative fallback (Transaction::add_table_with_flags TODO). Canonicalization-based rule diff + sets/maps + reconcile retry mode + recipe deferred |
