...
| [154](154-0.16-nlink-macros-plan.md) | New `nlink-macros` proc-macro crate: `#[derive(GenlMessage/GenlCommand/GenlAttribute/GenlEnum/NetlinkAttrs)]` + `#[genl_family]` | ~7.5 days | 9 | 🟡 | – | Phases 1+2+3a+3b+4+5 done: 5 derive/attr macros + `GenlFamily` send-time trait + `Connection::<F: GenlFamily>::send_typed<M, R>` / `dump_typed_stream<M, R>` generic dispatch. Downstream code now declares a complete GENL family AND its full request/response round-trip via the macros (`conn.send_typed(req).await?`). 23 runtime tests in nlink::macros + 14 in nlink-macros. nlink → nlink-macros wired. Remaining: `#[derive(NetlinkAttrs)]` (nested-group support — pure follow-up) + worked example + recipe (Phase 6) + final re-export polish (Phase 7). **Publish before nlink** when the cycle cuts |
